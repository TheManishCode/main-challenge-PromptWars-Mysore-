...
- **Today hub (new default, `TodayHub`):** orients instead of dumping into a section — time-aware greeting ("Good evening, …" / "Still up?" after 9pm), the 10-second check-in as the centerpiece, a one-line inline snapshot (check-ins · mood · sleep · burnout, color-coded), and a "Where to next?" set of 4 purposeful shortcut cards that route to the other areas. Avoids the identical-card-grid / hero-metric traps.
- **Topbar decluttered:** now `brand orb + wordmark | centered pill nav | theme-icon + Settings + account`. Removed the duplicate burnout badge and the separate full-width status-strip (snapshot moved into Today). Theme toggle is now a round icon (☾/☀) instead of a text button. Nav drops to its own full-width line under 780px.
- Removed the now-orphaned `Stat` component. Verified: `npm run verify` → lint clean, 45/45 tests, build OK (EXIT=0, 23 routes). Dev + prod servers serve 200; visual screenshot still blocked by the in-environment preview renderer (Remotion splash overlay + harness can't execute the page — zero console logs, screenshot times out).

### Warm-Human Redesign + Check-in + Recall (2026-06-23)
- **Design foundation:** added `PRODUCT.md` (register=product; warm/calm/encouraging personality; anti-references = cold/clinical and busy/cluttered; 5 design principles) and `DESIGN.md` (full OKLCH token system, type, motion, layout) via the impeccable skill's init flow. Direction confirmed with the user: **warm & human**, keep all features, keep oneko + the API auto-detect system + the chat model picker untouched, add the 10-second check-in plus an education-helpful feature.
- **Fonts via `next/font`:** replaced the Google-Fonts `@import` with `next/font/google` in `layout.js` — **Fraunces** (soft optical serif) for display/headings, **Plus Jakarta Sans** (humanist sans) for all UI/body, **Caveat** kept for handwriting. Exposed as `--font-display/--font-sans/--font-hand` on `<html>`; `globals.css` maps these to `--font-head/--font-body/--font-script`. Verified at runtime: SSR ships the `fraunces`/`jakarta` font classes.
- **New design system (`globals.css`):** kept every CSS variable NAME stable (so the ~2k lines of component CSS and all feature wiring keep working) and swapped VALUES to a warm-human OKLCH palette — soft warm off-white bg (deliberately not the AI "cream"), calm eucalyptus-teal `--brand`, warm apricot-coral `--accent`, low-glare warm-charcoal dark theme. Added `--on-brand`, `--radius-lg/-pill`, `--ease`, a semantic z-index scale, softer shadows, global `:focus-visible` ring, `::selection`. Restyled primitives with a full state vocabulary (hover/active/focus), pill buttons + nav, calmer body background (2 soft washes instead of 3 strong radials), display-serif headings + stat numbers, softened the comic insight-bubbles into warm speech cards (kept the meaningful per-category accent), brand/relief/nav active states moved off black-ink to `--brand-dark`, landing + splash headings de-weighted for the serif. Calmed the section-nav into a wrapping pill bar (now 8 tabs).
- **New: 10-second check-in** (`CheckInSection`, default landing tab). Tap a mood face + optional energy/sleep chips + optional one-line note → composes an honest short journal string from the taps and saves through the existing `/api/entries` pipeline (no new backend, no mock data). Warm confirmation state ("You showed up today") with paths to chat/journal. Rest framed as progress, never streak-shaming.
- **New: AI Recall** (education + engaging, not brainrot). New `generateRecallQuestions()` in `gemini.js`, `recallSchema` in `validation.js`, and `POST /api/recall` (auth + same-origin + rate-limited, 15/hr) build a gentle active-recall self-quiz (3–5 Q&A with hints) from the student's REAL recent study context (or a topic they type). `RecallSection` shows one question at a time, reveal-answer, next — explicitly no scores/streaks/wrong-answers, framed as "prove you know more than the panic says". Helps studying and counters "I'm not retaining anything" anxiety.
- Verified: `npm run verify` → lint clean, 45/45 tests, build OK (EXIT=0, now 23 routes with `/api/recall`). Production server runtime: home 200, fonts in SSR, `/api/recall` returns 401 unauthenticated (route + auth gate live). Visual/pixel confirmation still blocked by the known in-environment preview instability (dev/preview server keeps dropping); structural + runtime checks all pass.

### Clean Settings + In-Chat Model Picker (2026-06-23)
- **Settings decluttered:** removed the big "Advanced — AI provider" block (the 18-chip provider grid + manual key/base-URL/model fields + Test/Insert buttons + active-card). Settings now shows just the paste-and-go `QuickKeyPanel` for the API key (plus language + study-buddy). One clean panel instead of a wall of providers.
- **Soft-fail manual picker:** `QuickKeyPanel` now caches the fetched model list to `localStorage` (`mindtrail-models`). On Add it auto-detects the provider, fetches `/v1/models`, and saves the best one. If the model auto-fetch fails AND the provider wasn't confidently detected, it shows a soft amber warning ("Couldn't auto-fetch models…") with a provider dropdown + base-URL + model-name input so the user picks manually. A confidently-detected native provider falls back to its sane default model instead of nagging. Multi-model results still show a model dropdown.
- **ChatGPT-style model switcher in chat:** new `ChatModelPicker` in the chat header. With a user key it renders a compact `<select>` of the cached/fetched models (lazy-fetches once if not cached, auto-selects best when none chosen) and persists the choice to `mindtrail-model` so every AI request uses it. With no key it shows the app-default model name read-only.
- **Dead code removed:** deleted the now-orphaned `POST /api/provider-test` route and `testProvider()` in `gemini.js` (only the removed Settings Test button used them), plus the orphaned CSS (`.provider-grid/-fields/-actions`, `.active-card*`, `.key-btn-ghost`, `.settings-note/-link`, `[data-kind]` chip variants). Added `.status-warn`, `.chat-model-bar/-select`, `.chat-heading-actions` styles.
- Verified: `npm run verify` → lint clean, 45/45 tests pass, build succeeds (EXIT=0, now 22 routes after removing provider-test). Dev server serves home 200 with content and no console errors; full visual/interaction check is blocked by the known in-environment preview limitation (blank live DOM despite valid SSR).
