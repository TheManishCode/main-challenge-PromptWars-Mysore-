...
- Clerk authentication is required before app access; configure email/password or OAuth in Clerk.
...
- Upstash-backed rate limiting in production for journal analysis, insight generation, guestbook, and companion chat routes.
- Per-entry "Invoke Suggestions" creates saved AI insight bubbles with a fixed taxonomy.
- Companion chat uses the user's complete available journal history as server-side context.
- Shared guestbook wall stores authenticated handwritten-style notes with stable scattered layout metadata.
