...
function pickMessage(burnoutRisk) {
  const pool = burnoutRisk != null && burnoutRisk > 60 ? CALMING : ENCOURAGEMENTS;
...
export default function SlimeBuddy({ burnoutRisk }) {
  ...
  // Initial position + reduced-motion check
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      if (meltingRef.current) return; // frozen as a puddle
  ...
  // Idle moods + occasional supportive lines
  useEffect(() => {
  ...
  const registerHit = useCallback(() => {
    ...
    if (c >= 4) melt();
    else if (c === 3) { setEmotionFor('angry', 1500); say('grrr! stop poking me!', 1600); }
    else { setEmotionFor('hurt', 700); }
