...
export async function saveEntry(userKey, entry) {
  const encryptedJournal = encryptText(entry.journal);
  const activePool = getPool();
  if (!activePool) {
    const list = memoryStore.get(userKey) || [];
    list.unshift({ ...publicEntry(entry), rawTextEncrypted: encryptedJournal });
  ...
  try {
    ...
    await client.query(
      `INSERT INTO wellness_entries
        (id, user_key, exam, mood, energy, sleep_hours, raw_text_encrypted, analysis)
